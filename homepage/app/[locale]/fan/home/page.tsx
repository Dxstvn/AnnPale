          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
              <h1 className="text-3xl font-bold bg-gradient-to-r from-purple-600 to-pink-600 bg-clip-text text-transparent">
                {isAuthenticated ? t('dashboard.welcomeBack', { name: user?.email?.split('@')[0] || 'User' }) : t('dashboard.welcome')}
              </h1>
              <p className="text-muted-foreground mt-1">
                {isAuthenticated
                  ? t('dashboard.discoverContent')
            ...
            {!isAuthenticated && (
              <div className="flex items-center gap-2">
                <Link href="/login">
                  <Button variant="outline">{tCommon('navigation.signIn')}</Button>
                </Link>
                <Link href="/signup">
                  <Button>{tCommon('navigation.joinNow')}</Button>
                </Link>
              </div>
            )}
          </div>
              ...
              {subscribedCreators.length > 0 ? (
                <div className="bg-white/95 backdrop-blur-sm rounded-xl border border-purple-100 shadow-sm p-6">
                  <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center gap-2">
                      ...
                      <h3 className="text-lg font-semibold">{t('dashboard.yourCreators')}</h3>
                    ...
                    <div className="mt-4 pt-4 border-t border-gray-100 flex items-center justify-end">
                      <p className="text-xs text-muted-foreground">
                        {subscribedCreators.length === 1
                          ? t('dashboard.followingCount', { count: 1 })
                          : t('dashboard.followingCountPlural', { count: subscribedCreators.length })}
                      </p>
                    </div>
                  )}
                </div>
                ...
                <Card className="border-dashed border-2 border-purple-200 bg-purple-50/50">
                  <CardContent className="p-8 text-center">
                    ...
                    <h3 className="text-lg font-semibold mb-2">{t('dashboard.startFollowing')}</h3>
                    <p className="text-sm text-muted-foreground mb-4">
                      {t('dashboard.discoverCreators')}
                    </p>
                    <Link href="/fan/explore">
                      <Button className="bg-gradient-to-r from-purple-600 to-pink-600 text-white">
                        <Sparkles className="h-4 w-4 mr-2" />
                        {t('dashboard.exploreCreators')}
                      </Button>
                    </Link>
                  </CardContent>
                </Card>
