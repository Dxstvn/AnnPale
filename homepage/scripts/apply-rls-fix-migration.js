...
try {
  ...
  const command = `PGPASSWORD=REDACTED psql -h aws-1-us-east-1.pooler.supabase.com -U postgres.yijizsscwkvepljqojkz -d postgres -p 6543 -f ${tempFile}`;
  ...
  try {
    const altCommand = `echo "${sqlContent.replace(/"/g, '\\"').replace(/\n/g, '\\n')}" | PGPASSWORD=REDACTED psql -h aws-1-us-east-1.pooler.supabase.com -U postgres.yijizsscwkvepljqojkz -d postgres -p 6543`;
